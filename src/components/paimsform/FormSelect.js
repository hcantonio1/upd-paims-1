...
export const AggregatedFormSelect = (props) => {
  const { id, label, options, choicenamegetter: getChoiceName } = props;
  const smallLabel = label.charAt(0).toLowerCase() + label.slice(1);
  const inputLabelLabel = `select-${smallLabel}`;
